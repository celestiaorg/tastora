...
func NewCollector(ctx context.Context, cfg Config, testName string, index int) (*Node, error) {
	...
	img := cfg.Image
    if img.Repository == "" {
        // Collector image runs as non-root; set uid:gid so our volume ownership matches
        img = container.NewImage("otel/opentelemetry-collector-contrib", "latest", "10001:10001")
    }
